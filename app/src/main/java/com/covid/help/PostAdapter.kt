...
class PostAdapter(private val activity: Activity, private val dataset: List<Post>) : RecyclerView.Adapter<PostAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.layout.likes_count.text = "Me gusta: ${likes.size}"
        holder.layout.name_Post.text  = post.userName
        holder.layout.post.text = post.post

        val sdf = SimpleDateFormat("dd/M/yyyy hh:mm a")

        holder.layout.date.text = sdf.format(post.date)
        setColor(liked, holder.layout.btn_like)

        holder.layout.btnShare.setOnClickListener {
        ...
        holder.layout.btn_like.setOnClickListener {
            liked = !liked
            setColor(liked, holder.layout.btn_like)
